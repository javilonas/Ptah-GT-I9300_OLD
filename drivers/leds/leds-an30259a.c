...
#define LED_MODE_BLINK_NORMAL	0
#define LED_MODE_FADE_NORMAL	1
#define LED_MODE_FADE_BRIGHT	2
#define LED_MODE_BLINK_BRIGHT	3
					...
					u8 brightness)
{
	...
	// only apply if not brightness (blink or fade) mode
	if ((led_enable_fade == LED_MODE_BLINK_NORMAL) || (led_enable_fade == LED_MODE_FADE_NORMAL))
	{
	...
	// enable fading mode if requested, otherwise stick to blinking
	if ((led_enable_fade == LED_MODE_FADE_NORMAL) || (led_enable_fade == LED_MODE_FADE_BRIGHT))
	{
                    ...
                    struct device_attribute *attr, char *buf)
{
	...
	// return current status of LED fading
	ret = sprintf(buf, "LED fading: %d\n", led_enable_fade);
	return ret;
}
					...
					const char *buf, size_t count)
{
	...
	// check for valid data and set global variable if ok
	if ((enabled >= LED_MODE_BLINK_NORMAL) && (enabled <= LED_MODE_BLINK_BRIGHT))
	{
